...
import ServicosPanel from "./servicos";
...
export default function BarbershopDashboard() {
  const [menuOpen, setMenuOpen] = useState(false); const [active, setActive] = useState<MenuKey>("inicio"); const [barbershop, setBarbershop] = useState<Barbershop | null>(null); const [loading, setLoading] = useState(true);
  useEffect(() => { const s = createClient(); let mounted = true; (async () => { const { data: u } = await s.auth.getUser(); if (!u.user) { window.location.href = "/"; return; } const { data } = await s.from("barbershops").select("id,name,responsible_name,whatsapp,email,city,state,number,address,cep").eq("owner_id", u.user.id).order("created_at", { ascending: false }).limit(1).maybeSingle(); if (mounted) { setBarbershop(data as Barbershop | null); setLoading(false); } })(); return () => { mounted = false; }; }, []);
  async function logout() { await createClient().auth.signOut(); window.location.href = "/"; }
  function selectMenu(k: MenuKey) { setActive(k); setMenuOpen(false); }
  const current = useMemo(() => menuItems.find((i) => i.key === active) ?? menuItems[0], [active]);
  const clientArea = () => { setMenuOpen(false); if (barbershop) window.location.href = `/barbearia/area-cliente?barbershop=${encodeURIComponent(barbershop.id)}`; };
  return <main className="barbershop-shell">
    <header className="barbershop-header"><div className="barbershop-brand" aria-label="Barba10"><div className="barbershop-logo">Barba<span>10</span></div><div className="barbershop-divider" /><div className="barbershop-name">{loading ? "Carregando..." : barbershop?.name || "Minha Barbearia"}</div></div><button className="menu-button" type="button" aria-label={menuOpen ? "Fechar menu" : "Abrir menu"} aria-expanded={menuOpen} onClick={() => setMenuOpen((o) => !o)}><span /><span /><span /></button></header>
